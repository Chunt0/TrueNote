  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl gap-0 overflow-hidden p-0">
        <div className="flex h-[80vh] flex-col md:h-[560px] md:flex-row">
          <nav className="flex w-full shrink-0 gap-0.5 overflow-x-auto border-b border-border bg-background/40 p-3 md:w-48 md:flex-col md:overflow-x-visible md:border-b-0 md:border-r">
            <DialogTitle className="hidden px-2 pb-2 pt-1 text-base md:block">Settings</DialogTitle>
            {sections.map((s) => (
              <button
                key={s.id}
                type="button"
                ...
                className={cn(
                  'flex shrink-0 items-center gap-2.5 whitespace-nowrap rounded-md px-2.5 py-2 text-left text-sm transition-colors',
                  activeId === s.id
          ...
          <div className="min-w-0 flex-1 overflow-y-auto p-4 sm:p-6 md:pr-10">
